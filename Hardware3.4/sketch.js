...
function setup() {
  ...
  connectButton.position(windowWidth/2 - 100, windowHeight/2 + 50);
  ...
  try {
    ...
    const checkIfOpened = setInterval(() => {
      if (port.opened()) {
        connected = true;
        connectButton.hide();
        console.log('✅ Serial connection established (confirmed with polling)!');
...
function draw() {
  ...
  let newX = parseInt(tokens[1]);
  let newY = parseInt(tokens[2]);
  ...
  if (!isNaN(newX) && !isNaN(newY)) {
    ...
    xPos = constrain(xPos, 0, windowWidth);
    yPos = constrain(yPos, 0, windowHeight);
  }

  if(tokens[2] === "Released") {
  ...
  if(connected){
    if(!gameStarted) {
      ...
      if(gameOver) {
        ...
        percPart.stop();
        startPart.start();
        if (joyStickPressed && restartTimer <= 0) {
          gameStarted = false;  
          gameOver = false;
          countDownActive = true;
          score = 0;
    ...
    textSize(48);
    text("Connect to Arduino", windowWidth/2 - 200, windowHeight/3);
    text("BUG SMASHER", windowWidth/2 - 180, windowHeight/2);
    part1.stop();
    percPart.stop();
    startPart.start();
...
function joyStickPressedGameState() {
  let hit = false;

  for (let i = ants.length - 1; i >= 0; i--) {
    if (ants[i].isSquished(xPos, yPos) && !ants[i].isSquishedAlready) {
      score++;
